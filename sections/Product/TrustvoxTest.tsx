...
const forceLoadTrustvoxScript = (storeId: string) => {
  ...
  setTimeout(() => {
    ...
    script.onload = () => {
      ...
      setTimeout(() => {
        const elements = document.querySelectorAll('[data-trustvox-product-code]');
        console.log(`Encontrados ${elements.length} elementos com data-trustvox-product-code`);
        elements.forEach((el, index) => {
          console.log(`Elemento ${index + 1}:`, el.getAttribute('data-trustvox-product-code'));
        });
      }, 2000);
    };
    script.onerror = () => {
