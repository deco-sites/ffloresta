...
const trustvoxScript = (storeId: string, productId: string, productName: string, productPhotos: string[]) => {
  ...
  setTimeout(() => {
    ...
    if (productId && productId.trim() !== '') {
      ...
      console.warn('Trustvox: Nenhum ID de produto válido encontrado, mostrando avaliações da loja');
    }
    ...
    if (widget) {
      widget.innerHTML = '<div class="flex items-center justify-center h-32 text-gray-500"><div class="text-center"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900 mx-auto mb-2"></div><p>Carregando avaliações...</p></div></div>';
    }
    ...
    script.onload = () => {
      console.log('Script Trustvox carregado. Configuração final:', window._trustvox);
    };
...
export default function TrustvoxReviews({ 
  ...
  if (finalProductId && finalProductId.length < 3) {
    console.warn('TrustvoxReviews - ID do produto muito curto:', finalProductId);
  }
  ...
  return (
    <div id="trustvox-reviews" class={`trustvox-reviews-section ${customClass}`}>
      {title && (
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: useScript(trustvoxScript, storeId, finalProductId, productName, productPhotos),
        }}
      />
    </div>
  );
